...
Before reading this article, you should read the [Introduction to Sia](/pages/sia/introduction/en.html) article and the technical side of renting article to have a better bearing of how everything works, and who is behind the project. While the top level ideas can be grasped pretty easily, the design behind Sia can be pretty complex, so keep that in mind.
...
>A `Skylink` is a 46 character string of base64 encoded data. This data represents a couple things, but for the most part, this link is storing the Merkle root of the file. Merkle roots are complicated and you can dive more into them in this paper written by the head of the Sia Foundation, Luke Champine. But basically, `Skylinks` are just unique file identifiers; every single combination of data (think video file) and metadata (think file size) has a unique `Merkle tree root`. To learn more about Skyfiles, see [here](/pages/skynet/concepts/en.html).
...
Unfortunately, as of today you cannot just run a Skynet portal in your browser (though this is being worked on by the [Sia Foundation](/pages/sia/foundation/en.html) with [Utreexo](https://forum.sia.tech/t/core-development-utreexo/54/14) support). Due to the many limitations of running a Sia full node talked about in the [Before You Start](/pages/renting/before-you-start/en.html) article, running a renter node is hard on your system. The main hard parts are:
...
>Side note, the registry is a database, it’s called [SkyDB](/pages/skynet/concepts/en.html) after all. But it is not a relational database like SQL, it’s a key-value store architecture. This can be somewhat limiting for some applications.

To learn more about the registry, see [here](/pages/skynet/concepts/en.html).
...
>Discover more [Skynet Apps](/pages/discover/skynet-apps/en.html).
...
This article is pretty much done, to learn more read about [building on skynet](/pages/skynet/building-on-skynet/en.html).
